...
Public Class FtpControl
    ...
    ''' <param name="I_FtpType" >FTPタイプ</param>
    ''' <param name="I_Ymd"     >日付</param>
    ''' <returns>TRUE|FALSE</returns>
    ''' <remarks></remarks> 
    Public Function Request(Optional ByVal I_TargetId As String = "",
                            Optional ByVal I_OrgCode As String = "",
                            Optional ByVal I_FtpType As String = "",
                            Optional ByVal I_Ymd As String = "") As Boolean

        Me.ERR = C_MESSAGE_NO.NORMAL
        Dim sm = New CS0050DBcon_bat
        sm.CS0050DBcon_bat()
        Dim ftpFiles As New CS0054FtpFiles(I_TargetId, sm.DBconStr)
        ...
        Try
            ...
            For Each serverID In ftpFiles.FTPFILES.Select(Function(x) x.ServerID)
                ...
                If Not _dicFtpClient.ContainsKey(serverID) Then
                    _dicFtpClient.Add(serverID, New CS0053FtpClient(serverID, sm.DBconStr))
                End If
            Next
            ...
            For Each ftpfile In ftpFiles.FTPFILES
                ...
                'パラメタの指定があれば、それを優先する
                If String.IsNullOrEmpty(I_FtpType) Then
                Else
                    ftpfile.FtpType = I_FtpType
                End If
                ...
                Select Case ftpfile.FtpType
                    ...
                    Case "RENAME"
                        'MGET実行
                        FtpRename(_dicFtpClient(ftpfile.ServerID), ftpfile, wkOrgCode, I_Ymd)
                End Select
            Next
    ...
    ''' <returns>TRUE|FALSE</returns>
    ''' <remarks></remarks> 
    Private Function FtpRename(ByRef ftpClient As CS0053FtpClient,
                            ByVal ftpFile As CS0054FtpFiles.FTP_FILE,
                            Optional ByVal orgCode As String = "",
                            Optional ByVal ymd As String = "") As Boolean
        ...
        Dim wkFiles = ftpDir.
                            Where(Function(x) x.FileType = CS0053FtpClient.FtpFileInfo.DirectoryEntryTypes.File AndAlso
                                                ...
                                                x.Extension = ftpFile.FileExt)
        For Each file In wkFiles
            Dim SelFile As String() = file.NameOnly.Split("_")
            ' リネーム（指定日以下のファイルをリネーム）
            If SelFile(1) <= ymd Then
                ftpClient.Rename(file.FullName, file.FullName & ".used")
            End If
        Next
    ...
    Private Sub PutLog(ByVal messageNo As String,
        ...
        Dim logWrite As New CS0054LOGWrite_bat With {
            .INFNMSPACE = "CB00015KoueiMaster",
            .INFCLASS = Me.GetType.Name,
